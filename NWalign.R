...
setwd('C:/Users/Simo/Desktop/UNIVERSITY/develop_packages/D.R.O.G.A')
...
G_align <- function(seq1, seq2, match, mismatch,gap, plot) {
  ...
  # may take a while for long strings
  if(plot == 'yes') {
    par(mar = c(4,4,3.5,3.5))
    break1 <- min(res[[1]])
    break2 <- max(res[[1]])
    break3 <- (min(res[[1]])+max(res[[1]]))/2
    break4 <- max(res[[1]])-1
    sort(unique(c(break1, break2, break3, break4,1, 0))) # compute the plot breaks
                                                        # to point out alignment direction
  
    plot(res[[1]], 
         breaks = sort(unique(c(break1, break2, break3, break4, 0))),
...
G_align(str1, str2, 1,-1,-1, plot = 'yes')
a
res<-Nw_scores(str1,str2,1,-1,-1)
res[1]
